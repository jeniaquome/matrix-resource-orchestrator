  ...
  if (conflicts.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 sm:p-6">
        <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-4">Resource Conflicts</h3>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 sm:p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-base sm:text-lg font-semibold text-slate-900">Resource Conflicts ({conflicts.length})</h3>
      </div>
        ...
        {conflicts.map((conflict) => (
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-0 mb-2">
              <span className="font-medium text-slate-900 text-sm sm:text-base">{conflict.resourceName}</span>
            ...
            <div className="space-y-1 mb-3">
              {conflict.projects.map((proj) => (
                <div key={proj.projectId} className="flex items-center gap-2 text-sm text-slate-600">
                  <ArrowRight className="w-3 h-3" />
                  <span>{proj.projectName}</span>
                  <span className="text-slate-400">({proj.hoursRequested}h/week)</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-start gap-2 text-xs sm:text-sm bg-amber-50 p-2 rounded border border-amber-100">
              ...
              <span className="text-slate-700">{conflict.recommendation}</span>
            </div>
          </div>
        ))}
      </div>
